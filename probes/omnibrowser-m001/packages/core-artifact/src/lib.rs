...
impl NTPXArtifact {
    pub fn new(
        mut object: NTPXObject,
        artifact_type: ArtifactType,
        content_hash: impl Into<String>,
    ) -> Self {
        object.schema_version = NTPX_ARTIFACT_SCHEMA.to_owned();
        Self {
            object,
            artifact_type,
    ...
    pub fn validate(&self) -> Result<(), CoreValidationError> {
        self.object.validate_as(NTPX_ARTIFACT_SCHEMA)?;
        validate_non_empty("artifact.content_hash", &self.content_hash)?;
        if matches!(
            self.release_state,
            ReleaseState::Signed | ReleaseState::Released
        ...
        for dependency in &self.dependencies {
            ...
            if dependency.artifact_id == self.object.id {
                return Err(CoreValidationError::SelfReference("artifact.dependencies"));
            }
...
mod tests {
    ...
    #[test]
    fn artifact_wire_format_flattens_object_contract() {
        let artifact = sample_artifact();
        let json = serde_json::to_value(artifact).unwrap();
        assert_eq!(json["schema_version"], NTPX_ARTIFACT_SCHEMA);
