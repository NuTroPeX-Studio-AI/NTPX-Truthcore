...
data class SourceAuthResult(val authenticated: Boolean, val reason: String)
...
object SourceAuthenticator {
    fun verify(evidence: Evidence, expectedHash: String? = null): SourceAuthResult {
        ...
        if (parsed.scheme.equals("memory", ignoreCase = true)) {
            return SourceAuthResult(true, "Local memory source is structurally authenticated")
        }
        if (!parsed.scheme.equals("https", ignoreCase = true)) {
            return SourceAuthResult(false, "Remote source is not HTTPS-authenticated")
        }
        if (parsed.host.isNullOrBlank()) return SourceAuthResult(false, "Source URI has no host")
