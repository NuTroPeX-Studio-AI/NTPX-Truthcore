...
/**
 * Semantic verification never invents a score when no verifier is installed.
 * A model/provider must explicitly report availability before its result can
 * strengthen ClaimLock.
 */
...
class SemanticVerifier(private val provider: NliProvider? = null) {
    fun verify(claim: String, evidence: String): SemanticScore {
        ...
        val engine = provider ?: return SemanticScore(
            available = false,
            reason = "No local NLI provider is configured; semantic confidence is unavailable",
        )
